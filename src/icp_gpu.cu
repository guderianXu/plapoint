...
std::atomic<int> g_icp_small_stats_step_kernel_launch_count{0};
std::atomic<int> g_icp_small_residual_stats_kernel_launch_count{0};
std::atomic<int> g_icp_small_terminal_alignment_residual_kernel_launch_count{0};
...
IcpDirectGridCellLookupShape makeIcpDirectGridCellLookupShape(
    const IcpGridCellBounds& bounds,
    int target_count,
    int cell_count)
{
    IcpDirectGridCellLookupShape shape{};
    if (target_count <= 0 || cell_count <= 0)
    {
        return shape;
    }
    if (bounds.min_x == INT_MIN || bounds.min_y == INT_MIN || bounds.min_z == INT_MIN ||
        bounds.max_x == INT_MAX || bounds.max_y == INT_MAX || bounds.max_z == INT_MAX)
    {
        return shape;
    }

    const std::int64_t range_x = static_cast<std::int64_t>(bounds.max_x) - bounds.min_x + 1;
    const std::int64_t range_y = static_cast<std::int64_t>(bounds.max_y) - bounds.min_y + 1;
    const std::int64_t range_z = static_cast<std::int64_t>(bounds.max_z) - bounds.min_z + 1;
    if (range_x <= 0 || range_y <= 0 || range_z <= 0 ||
        range_x > INT_MAX || range_y > INT_MAX || range_z > INT_MAX)
    {
        return shape;
    }
    if (range_x > std::numeric_limits<std::int64_t>::max() / range_y)
    {
        return shape;
    }
    const std::int64_t xy_count = range_x * range_y;
    if (xy_count > std::numeric_limits<std::int64_t>::max() / range_z)
    {
        return shape;
    }
    const std::int64_t entry_count = xy_count * range_z;
    const std::int64_t max_entries_by_points =
        std::max<std::int64_t>(1024, static_cast<std::int64_t>(target_count) * 4);
    const std::int64_t max_entries_by_cells =
        std::max<std::int64_t>(1024, static_cast<std::int64_t>(cell_count) * 4);
    const std::int64_t max_entries = std::min(max_entries_by_points, max_entries_by_cells);
    if (entry_count <= 0 || entry_count > INT_MAX || entry_count > max_entries)
    {
        return shape;
    }
    ...
    shape.min_z = bounds.min_z;
    shape.range_x = static_cast<int>(range_x);
    shape.range_y = static_cast<int>(range_y);
    shape.range_z = static_cast<int>(range_z);
    shape.entry_count = static_cast<int>(entry_count);
    shape.active = true;
    return shape;
}
...
__global__ void fillIcpDirectGridCellLookupKernel(
    const IcpGridCellKey* __restrict__ cell_keys,
    int cell_count,
    int min_x,
    int min_y,
    ...
    int range_y,
    int range_z,
    int* __restrict__ direct_lookup_cell_indices,
    int entry_count)
{
    ...
    const IcpGridCellKey key = loadIcpGridCellKey(cell_keys, cell_idx);
    const long long local_x = static_cast<long long>(key.x) - min_x;
    const long long local_y = static_cast<long long>(key.y) - min_y;
    const long long local_z = static_cast<long long>(key.z) - min_z;
    if (local_x < 0 || local_y < 0 || local_z < 0 ||
        local_y >= range_y || local_z >= range_z)
    {
        return;
    }

    const long long linear_idx = (local_x * range_y + local_y) * range_z + local_z;
    if (linear_idx < 0 || linear_idx >= entry_count)
    {
        return;
    }
    direct_lookup_cell_indices[static_cast<int>(linear_idx)] = cell_idx;
}
    ...
    int& local_y)
{
    ...
    local_x = static_cast<int>(local_x_u);
    local_y = static_cast<int>(local_y_u);
    return true;
}

__device__ __forceinline__ int directLookupIcpGridCellXyBaseFromLocal(
    const IcpTargetSpatialGrid& target_grid,
    int local_x,
    int local_y)
{
...
#endif
    return (local_x * target_grid.direct_lookup_range_y + local_y) * target_grid.direct_lookup_range_z;
}
    ...
    int& xy_base)
{
    int local_x = 0;
    int local_y = 0;
    if (!directLookupIcpGridCellXyLocal<CheckActive>(target_grid, query_x, query_y, local_x, local_y))
    {
        return false;
    }

    xy_base = directLookupIcpGridCellXyBaseFromLocal(target_grid, local_x, local_y);
...
template <bool CheckLinearBounds>
    ...
    int query_z)
{
    ...
    if constexpr (CheckLinearBounds)
    {
#ifdef PLAPOINT_ENABLE_TESTING
        atomicAdd(&g_icp_direct_grid_lookup_linear_guard_count, 1ull);
#endif
    ...
    return loadReadOnlyIcpValue(target_grid.direct_lookup_cell_indices + linear_idx);
}
...
{
#ifdef PLAPOINT_ENABLE_TESTING
    if (icpGridCellCoordinate(x, cell_size) == cell_x &&
...
__device__ __forceinline__ double minDistanceSqToIcpNeighborCellXY(
    ...
    int dy_offset_idx)
{
    if (dx_offset_idx == 0 && dy_offset_idx == 0)
    {
        return 0.0;
    }
#ifdef PLAPOINT_ENABLE_TESTING
    ...
    for (int offset = 0; offset < count; ++offset)
    {
        ...
        if (isfinite(dist_sq) && dist_sq <= best_dist_sq)
        {
            ...
            if (update_best)
            {
                ...
                best_ty = ty;
                best_tz = tz;
                if (CanStopAfterExactMatch && dist_sq <= 0.0)
                {
                    stop_cell_scan = true;
                    return;
                }
...
template <typename Scalar>
__device__ __forceinline__ void scanIcpResidualGridCellCandidates(
    const IcpTargetSpatialGrid& target_grid,
    int cell_idx,
    double sx,
    double sy,
...
#pragma unroll
            for (int dy_offset_idx = 0; dy_offset_idx < 3; ++dy_offset_idx)
            {
                ...
                if constexpr (DirectLookup)
                {
                    int direct_lookup_local_x = 0;
                    int direct_lookup_local_y = 0;
                    if (!directLookupIcpGridCellXyLocal<false>(
                            target_grid,
                            query_x,
                            query_y,
                            direct_lookup_local_x,
                            direct_lookup_local_y))
                    {
                        continue;
                    }
                    ...
                    const int direct_lookup_xy_base = directLookupIcpGridCellXyBaseFromLocal(
                        target_grid,
                        direct_lookup_local_x,
                        direct_lookup_local_y);
                    for (int z_offset_idx = 0; z_offset_idx < 3 && !stop_cell_scan; ++z_offset_idx)
                    {
                        ...
                        const int cell_idx =
                            directLookupIcpGridCellAtZ<false>(target_grid, direct_lookup_xy_base, query_z);
                        if (cell_idx >= 0)
                        {
                            const double min_cell_dist_sq =
                                min_xy_dist_sq + minDistanceSqToIcpNeighborCellZ<FiniteCellBounds>(
                                    sz,
                                    query_z,
                                    target_grid.cell_size,
                                    z_offset_idx);
                            ...
                            {
                                scanIcpGridCellCandidates<Scalar, can_stop_after_exact_match>(
                                    target_grid,
                                    cell_idx,
                                    sx,
                                    sy,
...
template <typename Scalar>
__global__ void collectTransformedExactPointwiseIdentityAlignmentStatsKernel(
                ...
                if constexpr (DirectLookup)
                {
                    ...
                    for (int z_offset_idx = 0; z_offset_idx < 3 && !stop_cell_scan; ++z_offset_idx)
                    {
                        ...
                        if (cell_idx >= 0)
                        {
                            ...
                            if (min_cell_dist_sq <= max_dist_sq && min_cell_dist_sq < best_dist_sq)
                            {
                                scanIcpResidualGridCellCandidates<Scalar>(
                                    target_grid,
                                    cell_idx,
                                    sx,
                                    sy,
...
template <typename Scalar>
__global__ void reduceRawIcpIdentityStatsAndSetExactPointwiseIdentityAccumulatedAlignmentStepKernel(
    const RawIcpIdentityStats* __restrict__ partial_stats,
    int partial_count,
    ...
    IcpAlignmentStepRawResult<Scalar>* __restrict__ result)
{
    ...
    const RawIcpIdentityStats raw = shared_stats[0];
    const int step_valid = raw.active_count > 0 && isfinite(raw.residual_sq_sum) ? 1 : 0;
    if (local_idx < 16)
    {
        const int row = local_idx & 3;
    ...
    if (local_idx == 0)
    {
        writeIdentityAlignmentStepRawResultFields<Scalar>(raw, result);
    }
}
...
__global__ void computeSmallTargetIcpAlignmentStepKernel(
    ...
    Scalar* __restrict__ accumulated_transform,
    IcpAlignmentStepRawResult<Scalar>* __restrict__ result,
    const IcpAlignmentStepRawResult<Scalar>* __restrict__ source_transform_alignment_step = nullptr)
{
    ...
    IcpTerminalAlignmentAndResidualRawResult<Scalar>* __restrict__ result)
{
    ...
    if (local_idx == 0)
    {
        writeAlignmentStepRawResultFromRawStats<Scalar>(
            shared_stats[0],
            step_transform,
            &result->alignment_step);
    ...
    if (residual_source_valid)
    {
        for (int target_idx = 0; target_idx < target_count; ++target_idx)
        {
            ...
            const double dx = fsx - target_tile_x[target_idx];
            const double dy = fsy - target_tile_y[target_idx];
            const double dz = fsz - target_tile_z[target_idx];
            const double dist_sq = dx * dx + dy * dy + dz * dz;
#ifdef PLAPOINT_ENABLE_TESTING
            atomicAdd(&g_icp_full_distance_evaluation_count, 1ull);
#endif
            if (isfinite(dist_sq) && dist_sq < residual_best_dist_sq)
            {
                residual_best_dist_sq = dist_sq;
                if (dist_sq <= 0.0)
                {
                    break;
                }
    ...
    if (residual_source_valid && isfinite(residual_best_dist_sq) && residual_best_dist_sq <= max_dist_sq)
    {
        residual_local.active_count = 1;
        residual_local.residual_sq_sum = residual_best_dist_sq;
    }

    __shared__ RawIcpResidualStats shared_residual_stats[kIcpStatsBlockSize];
    shared_residual_stats[local_idx] = residual_local;
    __syncthreads();
    ...
    for (int stride = kIcpStatsBlockSize / 2; stride > 0; stride >>= 1)
    {
        if (local_idx < stride)
        {
            addRawIcpResidualStats(shared_residual_stats[local_idx], shared_residual_stats[local_idx + stride]);
        }
        __syncthreads();
    }
    ...
    if (local_idx == 0)
    {
        result->residual_stats = shared_residual_stats[0];
    }
}

template <typename Scalar, bool TransformSource, bool TryTransformedExactPointwise>
__global__ void computeSmallTargetIcpResidualStatsKernel(
    const Scalar* __restrict__ source_transform,
    const Scalar* source_points,
    int source_count,
    const Scalar* target_points,
    ...
    Scalar max_correspondence_distance,
    Scalar* output_points,
    RawIcpResidualStats* __restrict__ result)
{
    const int source_idx = threadIdx.x;
    const int local_idx = threadIdx.x;
    RawIcpResidualStats local{};
    ...
    if (local_idx == 0)
    {
        *result = shared_stats[0];
    }
}

template <typename Scalar>
__global__ void computeSmallTargetIcpStatsAndStepKernel(
    const Scalar* source_points,
    int source_count,
    const Scalar* target_points,
    int target_count,
...
template <typename Scalar>
bool shouldUseSmallFiniteRadiusTargetKernel(
    Scalar max_correspondence_distance,
    int source_count,
    int target_count)
{
...
template <typename Scalar>
bool shouldUseSmallFiniteRadiusTargetTileCapacityKernel(
    Scalar max_correspondence_distance,
    int source_count,
    int target_count)
{
    ...
    return source_count > 0 &&
        source_count <= kIcpStatsBlockSize &&
        target_count > 0 &&
        target_count <= kIcpStatsBlockSize &&
        std::isfinite(max_dist) &&
        max_dist > 0.0;
}
...
template <typename Scalar>
bool launchSmallTargetStatsAndStep(
    const Scalar* d_source_points,
    int source_count,
    const Scalar* d_target_points,
    int target_count,
    ...
    cudaStream_t stream)
{
    if (!shouldUseSmallFiniteRadiusTargetKernel(max_correspondence_distance, source_count, target_count))
    {
        return false;
    }
    ...
    constexpr int block_size = kIcpStatsBlockSize;
#ifdef PLAPOINT_ENABLE_TESTING
    g_icp_small_stats_step_kernel_launch_count.fetch_add(1, std::memory_order_relaxed);
#endif
    computeSmallTargetIcpStatsAndStepKernel<Scalar><<<1, block_size, 0, stream>>>(
        d_source_points,
        source_count,
        d_target_points,
        target_count,
    ...
    cudaStream_t stream)
{
    ...
    constexpr int block_size = kIcpStatsBlockSize;
    auto* d_identity_partials = reinterpret_cast<RawIcpIdentityStats*>(d_partials);
    collectTransformedExactPointwiseIdentityAlignmentStatsKernel<Scalar>
    ...
    g_icp_exact_pointwise_identity_step_kernel_launch_count.fetch_add(1, std::memory_order_relaxed);
    g_icp_transformed_identity_alignment_step_count.fetch_add(1, std::memory_order_relaxed);
#endif
    if constexpr (AccumulateTransform)
    {
        reduceRawIcpIdentityStatsAndSetExactPointwiseIdentityAccumulatedAlignmentStepKernel<Scalar>
            <<<1, block_size, 0, stream>>>(
            d_identity_partials,
            partial_count,
            d_previous_accumulated_transform,
    ...
    {
        reduceRawIcpIdentityStatsAndSetExactPointwiseIdentityAlignmentStepKernel<Scalar>
            <<<1, block_size, 0, stream>>>(
            d_identity_partials,
            partial_count,
            d_step_transform,
...
template <typename Scalar, bool TransformSource>
bool launchSmallTargetResidualStats(
    const Scalar* d_source_transform,
    const Scalar* d_source_points,
    int source_count,
    const Scalar* d_target_points,
    ...
    cudaStream_t stream)
{
    ...
    constexpr int block_size = kIcpStatsBlockSize;
#ifdef PLAPOINT_ENABLE_TESTING
    g_icp_small_residual_stats_kernel_launch_count.fetch_add(1, std::memory_order_relaxed);
#endif
    if constexpr (TransformSource)
    {
        const bool try_transformed_exact_pointwise =
            detail::canProbeTransformedExactPointwiseStats(source_count, d_target_points, target_count, nullptr);
#ifdef PLAPOINT_ENABLE_TESTING
        recordTransformedExactPointwiseResidualCallForTesting(try_transformed_exact_pointwise);
#endif
        if (try_transformed_exact_pointwise)
        {
            computeSmallTargetIcpResidualStatsKernel<Scalar, true, true><<<1, block_size, 0, stream>>>(
                d_source_transform,
                d_source_points,
                source_count,
                d_target_points,
        ...
        {
            computeSmallTargetIcpResidualStatsKernel<Scalar, true, false><<<1, block_size, 0, stream>>>(
                d_source_transform,
                d_source_points,
                source_count,
                d_target_points,
    ...
    {
        computeSmallTargetIcpResidualStatsKernel<Scalar, false, false><<<1, block_size, 0, stream>>>(
            d_source_transform,
            d_source_points,
            source_count,
            d_target_points,
...
void populateTargetSpatialGridDirectLookup(
    IcpTargetSpatialGrid& grid,
    IcpCorrespondenceStatsWorkspace& workspace)
{
    const int entry_count = workspace.targetSpatialGridDirectLookupEntryCount();
    auto* d_direct_lookup = reinterpret_cast<int*>(workspace.targetSpatialGridDirectLookupStorage());
    if (!d_direct_lookup || entry_count <= 0)
    {
        return;
    }

    grid.direct_lookup_cell_indices = d_direct_lookup;
    grid.direct_lookup_min_x = workspace.targetSpatialGridDirectLookupMinX();
    grid.direct_lookup_min_y = workspace.targetSpatialGridDirectLookupMinY();
    grid.direct_lookup_min_z = workspace.targetSpatialGridDirectLookupMinZ();
    grid.direct_lookup_range_x = workspace.targetSpatialGridDirectLookupRangeX();
    grid.direct_lookup_range_y = workspace.targetSpatialGridDirectLookupRangeY();
    grid.direct_lookup_range_z = workspace.targetSpatialGridDirectLookupRangeZ();
    grid.direct_lookup_entry_count = entry_count;
    ...
    IcpTargetSpatialGrid& grid)
{
    ...
    workspace.reserveTargetSpatialGridDirectLookup(shape.entry_count);
    auto* d_direct_lookup = reinterpret_cast<int*>(workspace.targetSpatialGridDirectLookupStorage());
    PLAPOINT_CHECK_CUDA(cudaMemsetAsync(
        d_direct_lookup,
        0xff,
        static_cast<std::size_t>(shape.entry_count) * sizeof(int),
        stream));
    fillIcpDirectGridCellLookupKernel<<<icpStatsPartialCount(cell_count), kIcpStatsBlockSize, 0, stream>>>(
        d_cell_keys,
        cell_count,
        shape.min_x,
        shape.min_y,
...
template <typename Scalar>
IcpTerminalAlignmentAndResidualResult<Scalar> makeHostTerminalAlignmentAndResidualResult(
    const IcpTerminalAlignmentAndResidualRawResult<Scalar>& raw)
{
    IcpTerminalAlignmentAndResidualResult<Scalar> result;
    result.alignment_step = makeHostAlignmentStepResult(raw.alignment_step);
    result.residual_stats = makeHostResidualStats<Scalar>(raw.residual_stats);
    ...
    bool assume_ordered_correspondences)
{
    ...
    const bool small_target_stats = launchSmallTargetResidualStats<Scalar, false>(
        nullptr,
        d_source_points,
        source_count,
        d_target_points,
    ...
    bool reserve_workspace)
{
    ...
    const bool small_target_stats = launchSmallTargetResidualStats<Scalar, true>(
        d_transform,
        d_source_points,
        source_count,
        d_target_points,
...
template <typename Scalar>
bool launchTransformPointsAndComputeIcpResidualStatsWithTargetSpatialGridSnapshotColumnMajorImpl(
    const Scalar* d_transform,
    const Scalar* d_source_points,
    int source_count,
    Scalar max_correspondence_distance,
    ...
    cudaStream_t stream)
{
    ...
    residual_workspace.reserveResidualStats(source_count);
    ...
    target_grid.finite_cell_bounds = icpGridCellBoundsAreFinite(target_grid.cell_size);
    target_grid.cell_count = target_spatial_grid_cell_count;
    populateTargetSpatialGridDirectLookup(target_grid, target_grid_workspace);

    constexpr int block_size = kIcpStatsBlockSize;
    const int grid_size = icpStatsPartialCount(source_count);
    auto* d_partials = reinterpret_cast<RawIcpResidualStats*>(residual_workspace.partialStorage());
    auto* d_stats = reinterpret_cast<RawIcpResidualStats*>(residual_workspace.statsStorage());
    if (!d_partials || !d_stats || !residual_workspace.hostResultStorage())
    {
        throw std::invalid_argument("ICP GPU: residual-stats workspace is not reserved");
    }
    launchTransformAndCollectResidualStatsSpatialGridKernel(
        grid_size,
        block_size,
    ...
    bool assume_ordered_correspondences)
{
    ...
    const bool small_target_stats_step = launchSmallTargetStatsAndStep<Scalar>(
        d_source_points,
        source_count,
        d_target_points,
        target_count,
    ...
    cudaStream_t stream)
{
    ...
    auto* d_result = reinterpret_cast<IcpAlignmentStepRawResult<Scalar>*>(stats_workspace.statsStorage());
    return launchSmallTargetAlignmentStep<Scalar, false, false>(
        nullptr,
        d_source_points,
        source_count,
        d_target_points,
...
template <typename Scalar>
bool launchTransformedSmallTargetAlignmentStepAndAccumulateTransformColumnMajorImpl(
    const Scalar* d_source_transform,
    const Scalar* d_source_points,
    int source_count,
    const Scalar* d_target_points,
    ...
    const IcpAlignmentStepRawResult<Scalar>* d_source_transform_alignment_step = nullptr)
{
    ...
    auto* d_result = reinterpret_cast<IcpAlignmentStepRawResult<Scalar>*>(stats_workspace.statsStorage());
    return launchSmallTargetAlignmentStep<Scalar, true, true>(
        d_source_transform,
        d_source_points,
        source_count,
        d_target_points,
...
template <typename Scalar>
bool launchTransformedIcpAlignmentStepAndAccumulateTransformColumnMajorImpl(
    const Scalar* d_source_transform,
    const Scalar* d_source_points,
    int source_count,
    const Scalar* d_target_points,
    ...
    const IcpAlignmentStepRawResult<Scalar>* d_source_transform_alignment_step = nullptr)
{
    ...
    if (small_target_step)
    {
        return launchTransformedSmallTargetAlignmentStepAndAccumulateTransformColumnMajorImpl(
            d_source_transform,
            d_source_points,
            source_count,
            d_target_points,
    ...
    cudaStream_t stream)
{
    ...
    {
        throw std::invalid_argument("ICP GPU: two-step small-target workspaces must be distinct");
    }
    if (source_count <= 0 || target_count <= 0)
    {
        return false;
    ...
    const bool first_launched =
        launchSmallTargetAlignmentStepColumnMajorImpl(
            d_source_points,
            source_count,
            d_target_points,
            target_count,
    ...
    if constexpr (std::is_same_v<Scalar, float>)
    {
        second_step_workspace.reserveFloatAlignmentStep(source_count);
    }
    else
    {
        second_step_workspace.reserveDoubleAlignmentStep(source_count);
    }
    auto* d_first_result =
        reinterpret_cast<IcpAlignmentStepRawResult<Scalar>*>(first_step_workspace.statsStorage());
    auto* d_second_result =
        reinterpret_cast<IcpAlignmentStepRawResult<Scalar>*>(second_step_workspace.statsStorage());
    return launchSmallTargetAlignmentStep<Scalar, true, true>(
        d_first_step_transform,
        d_source_points,
        source_count,
...
template <typename Scalar>
IcpAlignmentStepResult<Scalar> copyAlignmentStepResultFromReservedWorkspaceImpl(
    IcpCorrespondenceStatsWorkspace& stats_workspace,
    cudaStream_t stream)
{
    auto* d_result = reinterpret_cast<IcpAlignmentStepRawResult<Scalar>*>(stats_workspace.statsStorage());
    auto* h_result = reinterpret_cast<IcpAlignmentStepRawResult<Scalar>*>(stats_workspace.hostResultStorage());
    if (!d_result)
    {
        throw std::invalid_argument("ICP GPU: alignment-step device result workspace is not reserved");
    }
    if (!h_result)
    {
        throw std::invalid_argument("ICP GPU: alignment-step host result workspace is not reserved");
...
copySmallTargetTwoStepAlignmentResultFromReservedWorkspacesImpl(
    IcpCorrespondenceStatsWorkspace& first_step_workspace,
    IcpCorrespondenceStatsWorkspace& second_step_workspace,
    cudaStream_t stream)
{
    ...
    auto* d_first_result =
        reinterpret_cast<IcpAlignmentStepRawResult<Scalar>*>(first_step_workspace.statsStorage());
    auto* h_first_result =
        reinterpret_cast<IcpAlignmentStepRawResult<Scalar>*>(first_step_workspace.hostResultStorage());
    auto* d_second_result =
        reinterpret_cast<IcpAlignmentStepRawResult<Scalar>*>(second_step_workspace.statsStorage());
    auto* h_second_result =
        reinterpret_cast<IcpAlignmentStepRawResult<Scalar>*>(second_step_workspace.hostResultStorage());
    if (!d_first_result)
    {
        throw std::invalid_argument("ICP GPU: first alignment-step device result workspace is not reserved");
    }
    if (!h_first_result)
    {
        throw std::invalid_argument("ICP GPU: first alignment-step host result workspace is not reserved");
    }
    if (!d_second_result)
    {
        throw std::invalid_argument("ICP GPU: second alignment-step device result workspace is not reserved");
    }
    if (!h_second_result)
    {
        throw std::invalid_argument("ICP GPU: second alignment-step host result workspace is not reserved");
    }
    ...
    PLAPOINT_CHECK_CUDA(cudaMemcpyAsync(
        ...
        stream));
#ifdef PLAPOINT_ENABLE_TESTING
    g_icp_alignment_step_host_result_copy_count.fetch_add(2, std::memory_order_relaxed);
    g_icp_host_synchronization_count.fetch_add(1, std::memory_order_relaxed);
#endif
    PLAPOINT_CHECK_CUDA(cudaStreamSynchronize(stream));

    IcpSmallTargetTwoStepAlignmentResult<Scalar> result;
    result.first_alignment_step = makeHostAlignmentStepResult<Scalar>(*h_first_result);
...
{
    if (&first_step_workspace == &second_step_workspace ||
        &first_step_workspace == &residual_workspace ||
        &second_step_workspace == &residual_workspace)
    {
        throw std::invalid_argument("ICP GPU: combined two-step residual workspaces must be distinct");
    }
    ...
    auto* h_residual_result =
        reinterpret_cast<RawIcpResidualStats*>(residual_workspace.hostResultStorage());
    if (!d_first_result || !h_first_result)
    {
        throw std::invalid_argument("ICP GPU: first alignment-step result workspace is not reserved");
    }
    if (!d_second_result || !h_second_result)
    {
        throw std::invalid_argument("ICP GPU: second alignment-step result workspace is not reserved");
    }
    if (!d_residual_result || !h_residual_result)
    {
        throw std::invalid_argument("ICP GPU: residual-stats result workspace is not reserved");
    ...
    IcpTwoStepAlignmentAndResidualResult<Scalar> result;
    result.first_alignment_step = makeHostAlignmentStepResult<Scalar>(*h_first_result);
    result.second_alignment_step = makeHostAlignmentStepResult<Scalar>(*h_second_result);
    result.residual_stats = makeHostResidualStats<Scalar>(*h_residual_result);
    result.launched = true;
    ...
    cudaStream_t stream)
{
    ...
    PLAPOINT_CHECK_CUDA(cudaMemcpyAsync(
        h_result,
        d_result,
        sizeof(IcpTerminalAlignmentAndResidualRawResult<Scalar>),
        cudaMemcpyDeviceToHost,
        stream));
#ifdef PLAPOINT_ENABLE_TESTING
    g_icp_alignment_step_host_result_copy_count.fetch_add(1, std::memory_order_relaxed);
...
template <typename Scalar>
bool launchSmallTargetTwoStepTerminalAlignmentAndResidualColumnMajorImpl(
    const Scalar* d_source_points,
    int source_count,
    const Scalar* d_target_points,
    int target_count,
    ...
    Scalar* d_first_step_transform,
    Scalar* d_terminal_step_transform,
    Scalar* d_accumulated_transform,
    cudaStream_t stream,
    Scalar* d_output_points)
{
    if (&first_step_workspace == &terminal_workspace)
    {
        throw std::invalid_argument("ICP GPU: two-step small-target workspaces must be distinct");
    }
...
copySmallTargetTwoStepTerminalAlignmentAndResidualResultFromReservedWorkspacesImpl(
    IcpCorrespondenceStatsWorkspace& first_step_workspace,
    IcpCorrespondenceStatsWorkspace& terminal_workspace,
    cudaStream_t stream)
{
    ...
    auto* h_first_result =
        reinterpret_cast<IcpAlignmentStepRawResult<Scalar>*>(first_step_workspace.hostResultStorage());
    auto* d_terminal_result =
        reinterpret_cast<IcpTerminalAlignmentAndResidualRawResult<Scalar>*>(terminal_workspace.statsStorage());
    auto* h_terminal_result =
        reinterpret_cast<IcpTerminalAlignmentAndResidualRawResult<Scalar>*>(terminal_workspace.hostResultStorage());
    if (!d_first_result)
    {
        throw std::invalid_argument("ICP GPU: first alignment-step device result workspace is not reserved");
    }
    ...
    if (!d_terminal_result)
    {
        throw std::invalid_argument("ICP GPU: terminal alignment device result workspace is not reserved");
    }
    if (!h_terminal_result)
    {
        throw std::invalid_argument("ICP GPU: terminal alignment host result workspace is not reserved");
    }
    ...
    IcpSmallTargetTwoStepTerminalAlignmentAndResidualResult<Scalar> result;
    result.first_alignment_step = makeHostAlignmentStepResult<Scalar>(*h_first_result);
    result.terminal_result = makeHostTerminalAlignmentAndResidualResult<Scalar>(*h_terminal_result);
    result.launched = true;
    return result;
}
...
{
    g_icp_transformed_identity_alignment_step_count.store(0, std::memory_order_relaxed);
}

int icpTransformedIdentityAlignmentStepCountForTesting()
{
    return g_icp_transformed_identity_alignment_step_count.load(std::memory_order_relaxed);
}
...
void resetIcpTransformResidualOutputPointWriteCountForTesting()
{
    const unsigned long long zero = 0;
    PLAPOINT_CHECK_CUDA(
        cudaMemcpyToSymbol(g_icp_transform_residual_output_point_write_count, &zero, sizeof(zero)));
}
...
{
    g_icp_small_stats_step_kernel_launch_count.store(0, std::memory_order_relaxed);
}

int icpSmallStatsStepKernelLaunchCountForTesting()
{
    return g_icp_small_stats_step_kernel_launch_count.load(std::memory_order_relaxed);
}

void resetIcpSmallResidualStatsKernelLaunchCountForTesting()
{
    g_icp_small_residual_stats_kernel_launch_count.store(0, std::memory_order_relaxed);
}

int icpSmallResidualStatsKernelLaunchCountForTesting()
{
    return g_icp_small_residual_stats_kernel_launch_count.load(std::memory_order_relaxed);
}

void resetIcpSmallTerminalAlignmentResidualKernelLaunchCountForTesting()
{
    g_icp_small_terminal_alignment_residual_kernel_launch_count.store(0, std::memory_order_relaxed);
}

int icpSmallTerminalAlignmentResidualKernelLaunchCountForTesting()
{
    return g_icp_small_terminal_alignment_residual_kernel_launch_count.load(std::memory_order_relaxed);
}
...
{
    const unsigned long long zero = 0;
    PLAPOINT_CHECK_CUDA(cudaMemcpyToSymbol(g_icp_grid_cell_center_min_distance_count, &zero, sizeof(zero)));
}
...
{
    const unsigned long long zero = 0;
    PLAPOINT_CHECK_CUDA(cudaMemcpyToSymbol(g_icp_direct_grid_lookup_linear_guard_count, &zero, sizeof(zero)));
}
...
void IcpCorrespondenceStatsWorkspace::invalidateTargetSpatialGridCache()
{
    ...
    _target_spatial_grid_direct_lookup_min_x = 0;
    _target_spatial_grid_direct_lookup_min_y = 0;
    _target_spatial_grid_direct_lookup_min_z = 0;
    _target_spatial_grid_direct_lookup_range_x = 0;
    _target_spatial_grid_direct_lookup_range_y = 0;
    _target_spatial_grid_direct_lookup_range_z = 0;
...
void IcpCorrespondenceStatsWorkspace::reserveTargetSpatialGridDirectLookup(int entry_count)
{
    if (entry_count < 0)
    {
        throw std::invalid_argument("ICP GPU: target spatial-grid direct lookup count must not be negative");
    }
    if (entry_count == 0)
    {
        return;
    }
    if (targetSpatialGridDirectLookupCapacity() < entry_count)
    {
        _target_spatial_grid_direct_lookup_storage.allocate(static_cast<std::size_t>(entry_count) * sizeof(int));
    ...
    int entry_count)
{
    ...
    {
        throw std::invalid_argument("ICP GPU: target spatial-grid direct lookup cache metadata is invalid");
    }
    if (entry_count == 0)
    {
        _target_spatial_grid_direct_lookup_entry_count = 0;
        ...
        _target_spatial_grid_direct_lookup_range_y = 0;
        _target_spatial_grid_direct_lookup_range_z = 0;
        _target_spatial_grid_direct_lookup_evaluated = true;
        return;
    }
    if (range_x <= 0 || range_y <= 0 || range_z <= 0)
    {
        throw std::invalid_argument("ICP GPU: target spatial-grid direct lookup range must be positive");
    }

    _target_spatial_grid_direct_lookup_min_x = min_x;
    _target_spatial_grid_direct_lookup_min_y = min_y;
    _target_spatial_grid_direct_lookup_min_z = min_z;
    _target_spatial_grid_direct_lookup_range_x = range_x;
    _target_spatial_grid_direct_lookup_range_y = range_y;
    _target_spatial_grid_direct_lookup_range_z = range_z;
    _target_spatial_grid_direct_lookup_entry_count = entry_count;
    _target_spatial_grid_direct_lookup_evaluated = true;
}
...
IcpAlignmentStepResult<float> copyAlignmentStepResultFromReservedWorkspace<float>(
    ...
    cudaStream_t stream)
{
    return copyAlignmentStepResultFromReservedWorkspaceImpl<float>(
        stats_workspace,
        stream);
}

template <>
IcpAlignmentStepResult<double> copyAlignmentStepResultFromReservedWorkspace<double>(
    IcpCorrespondenceStatsWorkspace& stats_workspace,
    cudaStream_t stream)
{
    return copyAlignmentStepResultFromReservedWorkspaceImpl<double>(
        stats_workspace,
        stream);
}

template <>
IcpSmallTargetTwoStepAlignmentResult<float>
copySmallTargetTwoStepAlignmentResultFromReservedWorkspaces<float>(
    IcpCorrespondenceStatsWorkspace& first_step_workspace,
    IcpCorrespondenceStatsWorkspace& second_step_workspace,
    cudaStream_t stream)
{
...
template <>
IcpTwoStepAlignmentAndResidualResult<float>
copyIcpTwoStepAlignmentAndResidualResultFromReservedWorkspaces<float>(
    IcpCorrespondenceStatsWorkspace& first_step_workspace,
    IcpCorrespondenceStatsWorkspace& second_step_workspace,
    IcpCorrespondenceStatsWorkspace& residual_workspace,
    cudaStream_t stream)
{
    return copyIcpTwoStepAlignmentAndResidualResultFromReservedWorkspacesImpl<float>(
        first_step_workspace,
        second_step_workspace,
        residual_workspace,
        stream);
...
template <>
IcpTwoStepAlignmentAndResidualResult<double>
copyIcpTwoStepAlignmentAndResidualResultFromReservedWorkspaces<double>(
    IcpCorrespondenceStatsWorkspace& first_step_workspace,
    IcpCorrespondenceStatsWorkspace& second_step_workspace,
    IcpCorrespondenceStatsWorkspace& residual_workspace,
    cudaStream_t stream)
{
    return copyIcpTwoStepAlignmentAndResidualResultFromReservedWorkspacesImpl<double>(
        first_step_workspace,
        second_step_workspace,
        residual_workspace,
        stream);
...
bool launchSmallTargetTwoStepTerminalAlignmentAndResidualColumnMajorWithReservedWorkspaces(
    ...
    float* d_output_points)
{
    return launchSmallTargetTwoStepTerminalAlignmentAndResidualColumnMajorImpl(
        d_source_points,
        source_count,
        d_target_points,
        target_count,
...
template <>
IcpSmallTargetTwoStepTerminalAlignmentAndResidualResult<float>
copySmallTargetTwoStepTerminalAlignmentAndResidualResultFromReservedWorkspaces<float>(
    IcpCorrespondenceStatsWorkspace& first_step_workspace,
    IcpCorrespondenceStatsWorkspace& terminal_workspace,
    cudaStream_t stream)
{
    return copySmallTargetTwoStepTerminalAlignmentAndResidualResultFromReservedWorkspacesImpl<float>(
        first_step_workspace,
        terminal_workspace,
        stream);
}
...
template <>
IcpSmallTargetTwoStepTerminalAlignmentAndResidualResult<double>
copySmallTargetTwoStepTerminalAlignmentAndResidualResultFromReservedWorkspaces<double>(
    IcpCorrespondenceStatsWorkspace& first_step_workspace,
    IcpCorrespondenceStatsWorkspace& terminal_workspace,
    cudaStream_t stream)
{
    return copySmallTargetTwoStepTerminalAlignmentAndResidualResultFromReservedWorkspacesImpl<double>(
        first_step_workspace,
        terminal_workspace,
        stream);
}
