    ...
    Scalar* weights)
{
    ...
    for (int dy = 0; dy <= 1; ++dy)
    {
        for (int dx = 0; dx <= 1; ++dx)
        {
            ...
            atomicAddScalar(heights + cell, z * w);
            atomicAddScalar(weights + cell, w);
        }
    }
}

template <typename Scalar>
__global__ void normalizeHeightGridKernel(int cell_count, Scalar* heights, const Scalar* weights, std::uint8_t* valid)
{
    ...
    cudaStream_t stream)
{
    ...
    splatHeightGridKernel<Scalar><<<point_grid_size, block_size, 0, stream>>>(
        d_points, point_count, grid.width, grid.height, grid.minX, grid.minY, grid.stepX, grid.stepY,
        d_heights.get(), d_weights.get());
    PLAPOINT_CHECK_CUDA(cudaGetLastError());
    ...
    normalizeHeightGridKernel<Scalar><<<cell_grid_size, block_size, 0, stream>>>(
        cell_count_int, d_heights.get(), d_weights.get(), d_valid.get());
    PLAPOINT_CHECK_CUDA(cudaGetLastError());
