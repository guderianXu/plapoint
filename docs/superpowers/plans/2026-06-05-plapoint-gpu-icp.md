...
## Task 212: Reuse Same-Buffer Identity Result Across Calls

- Goal: skip the exact alignment-step kernel and compact host result sync on repeated GPU ICP calls when the source and
  target are the same GPU cloud object, the point buffer and version are unchanged, and the previous call already
  proved the identity result. This keeps the optimization narrower than a general align-result cache, so normal
  source/target pairs and non-identity benchmark rows still measure the ICP path.
- RED check:
  - Added `ICPGpuPathTest.AlignReusesSameBufferIdentityResultAcrossRepeatedCalls`.
  - Before implementation, two consecutive same-buffer identity `align(output)` calls produced two alignment-step
    calls, two exact pointwise step calls, and two host synchronizations. The desired path keeps those counts at one.
- Implementation:
  - Added a same-buffer identity-result cache keyed by source/target cloud object, point buffer, point version, point
    count, and the existing GPU accumulated transform buffer.
  - The fast path refreshes convergence, fitness, RMSE, and GPU final-transform validity from the cached identity
    result, then writes output only if the existing identity-output cache or point-buffer alias check cannot prove the
    output is already current.
  - The cache is marked only after a successful same-buffer identity alignment with full active coverage.
  - Added `ICPGpuPathTest.AlignRecomputesSameBufferIdentityAfterMutableSourceAccess` to prove mutable point access bumps
    the point version and forces the next align to recompute instead of reusing stale validation.
- Targeted verification:
  - `cmake --build build-codex-cuda -j$(nproc) &&
    ./build-codex-cuda/test/plapoint_tests --gtest_filter=ICPGpuPathTest.AlignReusesSameBufferIdentityResultAcrossRepeatedCalls`
    failed before implementation with alignment-step, exact-step, and host-sync counts of `2` vs expected `1`, then
    passed after implementation.
  - Same-buffer cache pair passed 2/2:
    `AlignReusesSameBufferIdentityResultAcrossRepeatedCalls` and
    `AlignRecomputesSameBufferIdentityAfterMutableSourceAccess`.
  - Identity/exact regression set passed 8/8:
    `AlignReusesSameBufferIdentityResultAcrossRepeatedCalls`,
    `AlignSkipsRepeatedIdentityOutputCopyWhenOutputIsUnchanged`,
    `AlignReusesIterationStatsForExactIdentityTerminalMetrics`,
    `AlignUsesExactPointwiseStatsForEqualInfiniteRadiusInputs`,
    `AlignDoesNotProbeExactPointwiseForEqualFiniteRadiusInputsByDefault`,
    `AlignCanProbeExactPointwiseForEqualFiniteRadiusInputsWhenEnabled`,
    `AlignmentStepPrefersSameBufferExactIdentityWhenOrdered`, and
    `AlignReusesExactNonIdentityStepForTerminalMetrics`.
  - `./build-codex-cuda/test/plapoint_tests --gtest_filter=ICPGpuPathTest.*`: 152/152 passed.
- Benchmark:
  - Rebuilt `build-codex-cuda-bench-only` and ran:
    `./build-codex-cuda-bench-only/benchmarks/plapoint_benchmarks --points 1000 --iterations 100 --icp-points 100000 --icp-max-iterations 3 --skip-cpu-icp`.
  - Relevant sample after this change:
    `gpu_icp_identity` = 0.679019 ms,
    `gpu_icp_identity_same_buffer_reuse_output` = 0.000136 ms,
    `gpu_icp_finite_radius` = 1.2124 ms,
    `gpu_icp_finite_radius_identity_reuse_output` = 0.210991 ms,
    `gpu_icp_finite_radius_identity_exact_probe_reuse_output` = 0.121036 ms,
    `gpu_icp_finite_radius_translation_reuse_output_one_iteration` = 0.70827 ms, and
    `gpu_icp_finite_radius_nonrigid_transform_only_two_iterations` = 1.42271 ms.
- Full verification:
  - `ctest --test-dir build-codex-cuda --output-on-failure`: 333/333 passed.
  - `cmake --build build-codex-cpu -j$(nproc) && ctest --test-dir build-codex-cpu --output-on-failure`: 143/144
    executed tests passed, with `plapoint.PointCloudTest.GpuTransfer` skipped by the CPU-only build.
  - `ctest --test-dir build-codex-cuda-bench-only --output-on-failure`: 1/1 passed.
  - `git diff --check`: clean before the full verification update.
...
## Task 230: Add Async Launch Building Block for Initial Small-Target Alignment Step

- Goal: complete the small-target async step trio needed by a future batched/asynchronous ICP surface. Task 228 covered
  terminal metrics, Task 229 covered transformed accumulated steps, and this task exposes the initial small-target
  alignment step as an enqueue-only detail API with explicit compact result collection.
- RED checks:
  - Added `ICPGpuPathTest.SmallTargetAlignmentStepAsyncLaunchDefersHostSynchronization`.
  - The RED failed at compile time because
    `launchSmallTargetAlignmentStepColumnMajorWithReservedWorkspace()` did not exist.
  - Added benchmark row
    `gpu_icp_alignment_step_small_finite_radius_target_async_launch_below_grid_threshold` to the row checker before
    implementation. The checker failed because the row was missing.
- Implementation:
  - Added float/double detail overloads for
    `launchSmallTargetAlignmentStepColumnMajorWithReservedWorkspace()`. They validate the small finite-radius
    predicate, reserve alignment-step workspace, enqueue the existing small-target alignment-step kernel, and return
    without copying the compact result to host or synchronizing the stream.
  - Reused `copyAlignmentStepResultFromReservedWorkspace<Scalar>()` from Task 229 for explicit result collection.
  - Added benchmark row
    `gpu_icp_alignment_step_small_finite_radius_target_async_launch_below_grid_threshold`, timing enqueue plus explicit
    stream synchronization but no host result copy.
- Correctness coverage:
  - The new test compares the async launch result and device step transform against the existing synchronous
    `computeIcpAlignmentStepColumnMajorWithReservedWorkspace()` helper. It asserts one small-target alignment kernel
    launch, zero alignment-step host result copies, and zero host synchronizations before explicit result copy; after
    copying, both counters are exactly one.
- Verification evidence:
  - The RED compile failure reproduced before implementation.
  - `ICPGpuPathTest.SmallTargetAlignmentStepAsyncLaunchDefersHostSynchronization`: 1/1 passed after implementation.
  - `ctest --test-dir build-codex-cuda-bench-only --output-on-failure`: 1/1 passed after adding the benchmark row.
- Benchmark:
  - 3-iteration sample with `--icp-points 1024`:
    `gpu_icp_alignment_step_small_finite_radius_target_below_grid_threshold` = 0.6011 ms,
    `gpu_icp_alignment_step_small_finite_radius_target_async_launch_below_grid_threshold` = 0.083866 ms,
    `gpu_icp_small_finite_radius_nonrigid_transform_only_two_iterations_below_grid_threshold` = 0.175769 ms,
    `gpu_icp_small_finite_radius_transformed_accumulated_async_launch_below_grid_threshold` = 0.085176 ms,
    and `gpu_icp_small_finite_radius_terminal_async_launch_below_grid_threshold` = 0.121389 ms.
- Current conclusion:
...
## Task 238: Add Async Launch Building Block for Large-Target Alignment Step

- Goal: start the large-target spatial-grid async split by exposing an enqueue-only detail helper for the initial
  compact alignment step. This is the large-target counterpart to the existing small-target launch/copy split and is a
  prerequisite for queuing multi-step or batched ICP work before collecting host convergence data.
- RED check:
  - Added `ICPGpuPathTest.AlignmentStepAsyncLaunchUsesSpatialGridAndDefersHostSynchronization`.
  - The RED build failed because `plapoint::gpu::detail::launchIcpAlignmentStepColumnMajorWithReservedWorkspace()`
    did not exist.
- Implementation:
  - Added float/double detail overloads of `launchIcpAlignmentStepColumnMajorWithReservedWorkspace()`.
  - The helper enqueues the existing generic initial alignment-step path and leaves result collection to
    `copyAlignmentStepResultFromReservedWorkspace()`.
  - It supports ordinary large finite-radius spatial-grid/fallback work and the same-buffer exact case; calls that
    would require ordered correspondences or host-guided exact-pointwise preflight fallback return `false` instead of
    changing fallback semantics.
  - Added benchmark row `gpu_icp_alignment_step_finite_radius_translation_async_launch_cached_grid`.
- Verification:
  - New spatial-grid async launch test: passed.
  - New async launch boundary test for unsupported host-guided fallback requests: passed.
  - Focused async launch subset: 2/2 passed.
  - Full `ICPGpuPathTest.*`: 191/191 passed.
  - Full CUDA CTest: 372/372 passed.
  - CPU build plus CTest: build passed; 144 CTest entries, 0 failed, with `plapoint.PointCloudTest.GpuTransfer`
    skipped in the CPU-only build.
  - Bench-only CTest: 1/1 passed.
- Benchmark:
  - 10-iteration sample with `--icp-points 1024`:
    `gpu_icp_alignment_step_finite_radius_translation_cached_grid` = 0.055493 ms,
    `gpu_icp_alignment_step_finite_radius_translation_cached_grid_reserved_workspace` = 0.055453 ms,
    and `gpu_icp_alignment_step_finite_radius_translation_async_launch_cached_grid` = 0.055491 ms.
- Current conclusion:
...
- Implementation:
  ...
  - Target-alias output uses the same target spatial-grid snapshot for residual collection, then writes the transformed
    points into the target output buffer after the combined copy synchronizes and the host validates the alignment
    result.
...
## Task 252: Add Large-Target Single-Step Source-Alias Final-Metrics Coverage

- Goal: extend the large-target, finite-radius, `maxIterations == 1`, finalMetrics=true combined-copy path to
  `align(*source)` while avoiding stale transform-cache reuse after the source cloud is overwritten.
- RED check:
  - Added
    `ICPGpuPathTest.AlignLargeTargetSingleIterationFinalMetricsWithSourceAliasAvoidsExtraHostSynchronization`.
  - RED failed as intended: the old source-alias path used two host synchronizations.
  - The same RED also exposed a stale-cache hazard: after `align(*source)`, a second `align()` reused the full-coverage
    transform cache for the pre-overwrite source and returned a non-identity transform without launching an alignment
    step.
- Implementation:
  - Allowed source-alias output in `tryAlignGpuSingleStepFinalMetrics()` when the source output buffer can be reused.
  - For source-alias and target-alias outputs, the final residual stats kernel now runs with no output pointer. The
    output transform is written only after the combined alignment/residual copy synchronizes and preconditions pass.
  - Source-alias and target-alias outputs both skip full-coverage transform/output cache population because one of the
    inputs has been overwritten.
  - Added `gpu_icp_finite_radius_translation_source_output_one_iteration` to the benchmark suite.
- Verification:
  - Source-alias RED: failed with host synchronizations 2 vs expected 1 and alignment-step reuse 0 vs expected 1 on the
    second align.
  - Source-alias test after implementation: passed.
  - Focused single-step/exact regression subset: 11/11 passed.
  - Full `ICPGpuPathTest.*`: 203/203 passed.
  - Full CUDA CTest: 384/384 passed.
  - CPU build plus CTest: build passed; 144 CTest entries, 0 failed, with `plapoint.PointCloudTest.GpuTransfer`
    skipped in the CPU-only build.
  - Bench-only CTest: 1/1 passed.
- Benchmark:
  - 5-iteration sample with `--icp-points 10000`:
    `gpu_icp_finite_radius_translation_reuse_output_one_iteration` = 0.000211 ms,
    `gpu_icp_finite_radius_translation_no_output_one_iteration` = 0.785654 ms,
    `gpu_icp_finite_radius_translation_target_output_one_iteration` = 0.820172 ms,
    `gpu_icp_finite_radius_translation_source_output_one_iteration` = 0.783031 ms,
    `gpu_icp_finite_radius_translation_reuse_output_skip_final_metrics_one_iteration` = 0.000204 ms,
    `gpu_icp_finite_radius_nonrigid_output_final_metrics_two_iterations` = 0.333411 ms,
    `gpu_icp_finite_radius_nonrigid_target_alias_final_metrics_two_iterations` = 0.348674 ms, and
    `gpu_icp_alignment_step_two_step_async_launch_separate_workspaces` = 0.23928 ms.
- Current conclusion:
