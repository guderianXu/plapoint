...
- **ICP GPU path** (`src/icp_gpu.cu`) — `IterativeClosestPoint<Scalar, GPU>` keeps source/target point buffers on GPU, computes correspondences with shared-memory target tiling, accumulates centroid/covariance/residual stats with block-level reductions, applies point transforms through caller-owned GPU output buffers, and accumulates the final 4x4 transform on GPU. The small 3x3 Kabsch SVD remains CPU-side, and `getFinalTransformationDevice()` exposes the final transform without forcing callers through the CPU copy. The stats helper can skip per-source correspondence index output when callers only need aggregate ICP moments, and `IcpCorrespondenceStatsWorkspace` lets repeated calls reuse device reduction storage.
