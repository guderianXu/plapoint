...
- **ICP GPU path** (`src/icp_gpu.cu`) — `IterativeClosestPoint<Scalar, GPU>` keeps source/target point buffers on GPU, computes correspondences, centroid/covariance/residual stats, applies point transforms, and accumulates the final 4x4 transform on GPU. The small 3x3 Kabsch SVD remains CPU-side, and `getFinalTransformationDevice()` exposes the final transform without forcing callers through the CPU copy.
