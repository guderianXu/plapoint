...
private:
    ...
    alignGpu(PointCloudType* output)
    {
        ...
        if (tryReuseGpuSameBufferIdentityResult(output, source_count, source_points, output_aliases_target))
        {
            return;
        }
        ...
        if (tryAlignGpuSmallTargetSingleStepTerminal(
                output,
                source_count,
                target_count,
                source_points,
        ...
        if (tryAlignGpuThreeStepTransformOnly(
                output,
                source_count,
                target_count,
                source_points,
        ...
        if (tryAlignGpuTwoStepFinalMetrics(
                output,
                source_count,
                target_count,
                source_points,
        ...
        if (tryAlignGpuThreeStepFinalMetrics(
                output,
                source_count,
                target_count,
                source_points,
        ...
        if (tryAlignGpuFiveStepFinalMetrics(
                output,
                source_count,
                target_count,
                source_points,
        ...
        if (tryAlignGpuSixStepFinalMetrics(
                output,
                source_count,
                target_count,
                source_points,
        ...
        if (tryAlignGpuSevenStepFinalMetrics(
                output,
                source_count,
                target_count,
                source_points,
        ...
        for (int iter = 0; iter < _max_iter; ++iter)
        {
            ...
            if (terminal_identity_step)
            {
                ...
                if (canCacheGpuSameBufferIdentityResult(source_count, source_points, target_points, stats.active_count))
                {
                    markGpuSameBufferIdentityResultCache(source_count, source_points);
                }
                else
                {
        ...
        bool output_aliases_target)
    {
        ...
        const double max_corr_dist = static_cast<double>(_max_corr_dist);
        if (source_count <= 0 ||
            source_count > small_target_tile_capacity ||
            target_count <= 0 ||
            target_count > small_target_tile_capacity ||
            source_points == target_points ||
            !std::isfinite(max_corr_dist) ||
        ...
        reserveGpuAlignmentStepWorkspace(source_count);
        const bool launched =
            gpu::detail::launchSmallTargetTwoStepAlignmentColumnMajorWithReservedWorkspaces(
                source_points,
                source_count,
                target_points,
                target_count,
        ...
        bool output_aliases_target)
    {
        ...
        {
            std::swap(_gpu_T_acc, _gpu_T_step);
            updateResidualMetricsFromGpuStats(first_step, source_count);
            _converged = first_step.active_count >= 3 && _fitness_score >= _min_fitness_score;
            invalidateGpuSameBufferIdentityResultCache();
            invalidateGpuExactIdentityResultCache();
            const double first_step_residual_sq_sum =
            ...
            if (!output_aliases_source &&
                ...
                canCacheGpuFullCoverageTransformResult(
                    ...
                    source_points,
                    target_points,
                    first_step.active_count,
                    first_step.step_maps_correspondences_exactly,
                    first_step.all_correspondences_same_index,
                    first_step.step.delta != Scalar(0),
                    first_step_residual_sq_sum))
            {
                markGpuFullCoverageTransformResultCache(source_count, target_count, source_points, target_points);
                invalidateGpuFullCoverageTransformOutputCache();
                invalidateGpuIdentityOutputCache();
        ...
        invalidateGpuSameBufferIdentityResultCache();
        invalidateGpuExactIdentityResultCache();
        updateResidualMetricsFromGpuStats(second_step, source_count);
        const bool second_step_converged = second_step.step.delta < _eps;
        if (second_step_converged)
        {
            _converged = second_step.active_count >= 3 && _fitness_score >= _min_fitness_score;
        }
        const double second_step_residual_sq_sum =
        ...
        if (!output_aliases_source &&
            ...
            canCacheGpuFullCoverageTransformResult(
                ...
                source_points,
                target_points,
                second_step.active_count,
                second_step.step_maps_correspondences_exactly,
                second_step.all_correspondences_same_index,
                true,
        ...
        bool output_aliases_target)
    {
        ...
        auto two_step_result =
            gpu::detail::copyIcpTwoStepAlignmentResultFromReservedWorkspaces<Scalar>(
                _gpu_stats_workspace,
                _gpu_terminal_stats_workspace);
        handleGpuAlignmentStepPreconditions(two_step_result.first_alignment_step, 0);
        ...
        handleGpuAlignmentStepPreconditions(two_step_result.second_alignment_step, 1);
        ...
        bool output_aliases_target)
    {
        ...
        const bool terminal_step_converged = second_step.step.delta < _eps;
        if (terminal_step_converged)
        {
            _converged = final_stats.active_count >= 3 && _fitness_score >= _min_fitness_score;
        }
        ...
        bool output_aliases_target)
    {
        ...
        const bool terminal_launched =
            gpu::detail::
                launchTransformedIcpTerminalAlignmentAndResidualWithTargetSpatialGridSnapshotColumnMajorWithReservedWorkspaces(
                    _gpu_T_acc->data(),
                    source_points,
                    source_count,
                    target_points,
        ...
        {
            throw std::runtime_error("ICP: three-step terminal final metrics were not launched");
        }
        const auto terminal_result =
            gpu::detail::copyIcpAlignmentAndResidualResultFromReservedWorkspaces<Scalar>(
                _gpu_stats_workspace,
                _gpu_final_stats_workspace);
        const auto& terminal_step = terminal_result.alignment_step;
        handleGpuAlignmentStepPreconditions(terminal_step, 2);
        std::swap(_gpu_T_acc, _gpu_next_T_acc);
        invalidateGpuSameBufferIdentityResultCache();
        invalidateGpuExactIdentityResultCache();
        const auto& final_stats = terminal_result.residual_stats;
        ...
        bool output_aliases_target)
    {
        ...
        if (!second_batch_launched)
        {
            throw std::runtime_error("ICP: five-step intermediate alignment batch was not launched");
        }
        two_step_result =
            gpu::detail::copyIcpTwoStepAlignmentResultFromReservedWorkspaces<Scalar>(
                _gpu_stats_workspace,
        ...
        const auto& fourth_step = two_step_result.second_alignment_step;
        handleGpuAlignmentStepPreconditions(fourth_step, 3);
        if (fourth_step.step.delta < _eps)
        {
            invalidateGpuSameBufferIdentityResultCache();
        ...
        if (!terminal_launched)
        {
            throw std::runtime_error("ICP: five-step terminal final metrics were not launched");
        }
        const auto terminal_result =
            gpu::detail::copyIcpAlignmentAndResidualResultFromReservedWorkspaces<Scalar>(
                _gpu_stats_workspace,
                _gpu_final_stats_workspace);
        const auto& fifth_step = terminal_result.alignment_step;
        handleGpuAlignmentStepPreconditions(fifth_step, 4);
        std::swap(_gpu_T_acc, _gpu_next_T_acc);
        invalidateGpuSameBufferIdentityResultCache();
        invalidateGpuExactIdentityResultCache();
        const auto& final_stats = terminal_result.residual_stats;
        ...
        if (fifth_step.step.delta < _eps)
        {
            _converged = final_stats.active_count >= 3 && _fitness_score >= _min_fitness_score;
        }
        const double fifth_step_residual_sq_sum =
            std::isfinite(fifth_step.step_residual_sq_sum)
                ? std::max(0.0, fifth_step.step_residual_sq_sum)
                : fifth_step.step_residual_sq_sum;
        if (canCacheGpuFullCoverageTransformResult(
                source_count,
                target_count,
                source_points,
                target_points,
                fifth_step.active_count,
                fifth_step.step_maps_correspondences_exactly,
                fifth_step.all_correspondences_same_index,
                true,
                fifth_step_residual_sq_sum))
        {
            markGpuFullCoverageTransformResultCache(source_count, target_count, source_points, target_points);
            invalidateGpuFullCoverageTransformOutputCache();
            invalidateGpuIdentityOutputCache();
        ...
        bool output_aliases_target)
    {
        ...
        if (!second_batch_launched)
        {
            throw std::runtime_error("ICP: six-step intermediate alignment batch was not launched");
        }
        two_step_result =
            gpu::detail::copyIcpTwoStepAlignmentResultFromReservedWorkspaces<Scalar>(
                _gpu_stats_workspace,
        ...
        const bool third_batch_launched =
            gpu::detail::launchTransformedIcpTwoStepAlignmentColumnMajorWithReservedWorkspaces(
                _gpu_T_acc->data(),
                source_points,
                source_count,
                ...
                _gpu_next_T_acc->data(),
                _gpu_T_acc->data());
        if (!third_batch_launched)
        {
            throw std::runtime_error("ICP: six-step terminal alignment batch was not launched");
        }
        const bool residual_launched =
            gpu::detail::
                launchTransformPointsAndComputeIcpResidualStatsWithTargetSpatialGridSnapshotColumnMajorWithReservedWorkspaces(
        ...
        if (!residual_launched)
        {
            throw std::runtime_error("ICP: six-step terminal final metrics were not launched");
        }
        const auto combined_result =
            gpu::detail::copyIcpTwoStepAlignmentAndResidualResultFromReservedWorkspaces<Scalar>(
                _gpu_stats_workspace,
                _gpu_terminal_stats_workspace,
                _gpu_final_stats_workspace);
        const auto& fifth_step = combined_result.first_alignment_step;
        handleGpuAlignmentStepPreconditions(fifth_step, 4);
        if (fifth_step.step.delta < _eps)
        {
            std::swap(_gpu_T_acc, _gpu_next_T_acc);
        ...
        const auto& sixth_step = combined_result.second_alignment_step;
        handleGpuAlignmentStepPreconditions(sixth_step, 5);
        invalidateGpuSameBufferIdentityResultCache();
        invalidateGpuExactIdentityResultCache();
        const auto& final_stats = combined_result.residual_stats;
        if (final_stats.invalid_source_count > 0)
        ...
        if (sixth_step.step.delta < _eps)
        {
            _converged = final_stats.active_count >= 3 && _fitness_score >= _min_fitness_score;
        }
        const double sixth_step_residual_sq_sum =
            std::isfinite(sixth_step.step_residual_sq_sum)
                ? std::max(0.0, sixth_step.step_residual_sq_sum)
                : sixth_step.step_residual_sq_sum;
        if (canCacheGpuFullCoverageTransformResult(
                source_count,
                target_count,
                source_points,
                target_points,
                sixth_step.active_count,
                sixth_step.step_maps_correspondences_exactly,
                sixth_step.all_correspondences_same_index,
                true,
                sixth_step_residual_sq_sum))
        {
            markGpuFullCoverageTransformResultCache(source_count, target_count, source_points, target_points);
            invalidateGpuFullCoverageTransformOutputCache();
            invalidateGpuIdentityOutputCache();
        ...
        bool output_aliases_target)
    {
        ...
        for (int batch_start_iter = 2; batch_start_iter < 6; batch_start_iter += 2)
        {
            const bool transformed_batch_launched =
                gpu::detail::launchTransformedIcpTwoStepAlignmentColumnMajorWithReservedWorkspaces(
                    _gpu_T_acc->data(),
            ...
            if (!transformed_batch_launched)
            {
                throw std::runtime_error("ICP: seven-step intermediate alignment batch was not launched");
            }
            two_step_result =
                gpu::detail::copyIcpTwoStepAlignmentResultFromReservedWorkspaces<Scalar>(
                    _gpu_stats_workspace,
                    _gpu_terminal_stats_workspace);
            const auto& first_batch_step = two_step_result.first_alignment_step;
            handleGpuAlignmentStepPreconditions(first_batch_step, batch_start_iter);
            if (first_batch_step.step.delta < _eps)
            {
                std::swap(_gpu_T_acc, _gpu_next_T_acc);
                invalidateGpuSameBufferIdentityResultCache();
                invalidateGpuExactIdentityResultCache();
            ...
            const auto& second_batch_step = two_step_result.second_alignment_step;
            handleGpuAlignmentStepPreconditions(second_batch_step, batch_start_iter + 1);
            if (second_batch_step.step.delta < _eps)
            {
                invalidateGpuSameBufferIdentityResultCache();
                invalidateGpuExactIdentityResultCache();
                const auto final_stats =
        ...
        if (!terminal_launched)
        {
            throw std::runtime_error("ICP: seven-step terminal final metrics were not launched");
        }
        const auto terminal_result =
            gpu::detail::copyIcpAlignmentAndResidualResultFromReservedWorkspaces<Scalar>(
                _gpu_stats_workspace,
                _gpu_final_stats_workspace);
        const auto& seventh_step = terminal_result.alignment_step;
        handleGpuAlignmentStepPreconditions(seventh_step, 6);
        std::swap(_gpu_T_acc, _gpu_next_T_acc);
        invalidateGpuSameBufferIdentityResultCache();
        invalidateGpuExactIdentityResultCache();
        const auto& final_stats = terminal_result.residual_stats;
        ...
        if (seventh_step.step.delta < _eps)
        {
            _converged = final_stats.active_count >= 3 && _fitness_score >= _min_fitness_score;
        }
        const double seventh_step_residual_sq_sum =
            std::isfinite(seventh_step.step_residual_sq_sum)
                ? std::max(0.0, seventh_step.step_residual_sq_sum)
                : seventh_step.step_residual_sq_sum;
        if (canCacheGpuFullCoverageTransformResult(
                source_count,
                target_count,
                source_points,
                target_points,
                seventh_step.active_count,
                seventh_step.step_maps_correspondences_exactly,
                seventh_step.all_correspondences_same_index,
                true,
                seventh_step_residual_sq_sum))
        {
            markGpuFullCoverageTransformResultCache(source_count, target_count, source_points, target_points);
            invalidateGpuFullCoverageTransformOutputCache();
            invalidateGpuIdentityOutputCache();
        ...
        bool output_aliases_target)
    {
        ...
        if (output_aliases_target && output && source_points == target_points)
        {
            return false;
        }
        const bool output_aliases_source = output == _source.get();
        ...
        const bool launched =
            gpu::detail::launchSmallTargetSingleStepTerminalAlignmentAndResidualColumnMajorWithReservedWorkspace(
                source_points,
                source_count,
                target_points,
                target_count,
        ...
        _final_T_gpu_valid = false;
        const auto terminal_result =
            gpu::detail::copySmallTargetTerminalAlignmentAndResidualResultFromReservedWorkspace<Scalar>(
                _gpu_stats_workspace);
        if (output_points && output_aliases_target)
        {
            invalidateGpuTargetWorkspaceCache();
        ...
        const auto& alignment_step = terminal_result.alignment_step;
        handleGpuAlignmentStepPreconditions(alignment_step, 0);
        std::swap(_gpu_T_acc, _gpu_T_step);
        invalidateGpuSameBufferIdentityResultCache();
        invalidateGpuExactIdentityResultCache();
        ...
        const bool terminal_step_converged = alignment_step.step.delta < _eps;
        if (terminal_step_converged)
        {
            _converged = final_stats.active_count >= 3 && _fitness_score >= _min_fitness_score;
        }
        ...
        if (!output_aliases_source &&
            ...
            canCacheGpuFullCoverageTransformResult(
                ...
                source_points,
                target_points,
                alignment_step.active_count,
                alignment_step.step_maps_correspondences_exactly,
                alignment_step.all_correspondences_same_index,
                alignment_step.step.delta != Scalar(0),
                terminal_step_residual_sq_sum))
        {
            markGpuFullCoverageTransformResultCache(source_count, target_count, source_points, target_points);
        ...
        bool output_aliases_target)
    {
        ...
        reserveGpuAlignmentStepWorkspace(source_count);
        const bool launched =
            gpu::detail::launchSmallTargetTwoStepTerminalAlignmentAndResidualColumnMajorWithReservedWorkspaces(
                source_points,
                source_count,
                target_points,
                target_count,
        ...
        _final_T_gpu_valid = false;
        const auto two_step_result =
            gpu::detail::copySmallTargetTwoStepTerminalAlignmentAndResidualResultFromReservedWorkspaces<Scalar>(
                _gpu_stats_workspace,
                _gpu_terminal_stats_workspace);
        handleGpuAlignmentStepPreconditions(two_step_result.first_alignment_step, 0);

        const bool first_step_terminal = two_step_result.first_alignment_step.step.delta < _eps;
        if (first_step_terminal)
        {
            std::swap(_gpu_T_acc, _gpu_T_step);
            finishGpuSingleStepTerminalMetrics(
        ...
        const auto& terminal_result = two_step_result.terminal_result;
        handleGpuAlignmentStepPreconditions(terminal_result.alignment_step, 1);
        invalidateGpuSameBufferIdentityResultCache();
        invalidateGpuExactIdentityResultCache();
        const auto& final_stats = terminal_result.residual_stats;
        if (final_stats.invalid_source_count > 0)
        ...
        const bool terminal_step_converged = terminal_result.alignment_step.step.delta < _eps;
        if (terminal_step_converged)
        {
            _converged = final_stats.active_count >= 3 && _fitness_score >= _min_fitness_score;
        }
        if (canCacheGpuFullCoverageTransformResult(
                ...
                source_points,
                target_points,
                terminal_result.alignment_step.active_count,
                terminal_result.alignment_step.step_maps_correspondences_exactly,
                terminal_result.alignment_step.all_correspondences_same_index,
                true,
                terminal_result.alignment_step.step_residual_sq_sum))
        {
            markGpuFullCoverageTransformResultCache(source_count, target_count, source_points, target_points);
            invalidateGpuFullCoverageTransformOutputCache();
            invalidateGpuIdentityOutputCache();
    ...
    void handleGpuAlignmentStepPreconditions(
        const gpu::IcpAlignmentStepResult<Scalar>& stats,
        int iter) const
    {
        if (stats.invalid_source_count > 0)
        {
        ...
        bool output_aliases_target)
    {
        ...
        if (canCacheGpuFullCoverageTransformResult(
                ...
                target_points,
                step.active_count,
                step.step_maps_correspondences_exactly,
                step.all_correspondences_same_index,
                step.step.delta != Scalar(0),
                step.step_residual_sq_sum))
        {
            markGpuFullCoverageTransformResultCache(source_count, target_count, source_points, target_points);
            invalidateGpuFullCoverageTransformOutputCache();
            invalidateGpuIdentityOutputCache();
    ...
    tryReuseGpuSameBufferIdentityResult(
        ...
        bool output_aliases_target)
    {
        if (!gpuSameBufferIdentityResultCacheMatches(source_count, source_points))
        {
            return false;
        }
    ...
    gpuSameBufferIdentityResultCacheMatches(int source_count, const Scalar* source_points) const
    {
        return _gpu_same_buffer_identity_result_cache_cloud == _source.get() &&
               ...
               _gpu_same_buffer_identity_result_cache_points_version == _source->pointsVersion() &&
               _gpu_T_acc != nullptr;
    }
    ...
    writeGpuIdentitySourceOutput(
        ...
        bool output_aliases_target)
    {
        if (gpuOutputAlreadyContainsIdentitySource(output, source_count, source_points))
        {
            return;
        }
        if (gpuOutputAlreadyContainsCurrentPoints(output, source_count, source_points))
        {
            markGpuIdentityOutputCache(output, source_count, source_points);
            return;
        }
        ...
        Scalar* output_points = prepareGpuOutputPointBuffer(output, source_count);
        if (output_points != source_points)
        {
            if (output_aliases_target)
            {
                invalidateGpuTargetWorkspaceCache();
    ...
    const PointCloudType* _gpu_same_buffer_identity_result_cache_cloud = nullptr;
